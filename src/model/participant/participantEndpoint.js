...
const getEndpoint = async (fsp) => {
  ...
  Logger.info(`[fsp=${fsp}] ~ Model::participantEndpoint::getEndpoint := fetching the endpoints from the resource with options: ${JSON.stringify(requestOptions)}`)
  return new Promise((resolve, reject) => {
    return request(requestOptions, (error, response, body) => {
      ...
      Logger.info(`[fsp=${fsp}] ~ Model::participantEndpoint::getEndpoints := Returning the endpoints: ${JSON.stringify(response)}`)
